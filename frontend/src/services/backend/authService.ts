...
import { JwtResponse, LoginRequest, RegisterRequest, User, MessageResponse } from './types';
...
export const authService = {
  ...
  login: (credentials: LoginRequest): Promise<JwtResponse> => {
    return apiClient.post<JwtResponse>('/auth/login', credentials, { requiresAuth: false });
  },
  ...
  register: (registerData: RegisterRequest): Promise<MessageResponse> => {
    return apiClient.post<MessageResponse>('/auth/register', registerData, { requiresAuth: false });
  },
