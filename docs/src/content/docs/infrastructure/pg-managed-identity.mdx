...
--- Example:
--- GRANT USAGE, SELECT ON SEQUENCE users_id_seq TO "clarity-poc-staging-backend";
--- This is used to grant the permissions to the sequence users_id_seq.
```

#### (Optional) Verify the AAD Role in PostgreSQL
...
#### (Optional) Drop the existing AAD Role in PostgreSQL
